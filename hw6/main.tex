...
\begin{enumerate}        
    ...
    {\checkboxchar{$\Box$} \checkedchar{$\blacksquare$}
        \begin{checkboxes}
        \CorrectChoice Increasing the order of the polynomial
        \choice Increasing the regularization weight
        \CorrectChoice For the same weight on the regularizer, using an L1 regularizer instead of an L2
        \CorrectChoice For the same weight on the regularizer, using an L1 regularizer instead of an L0
        \choice None of the above
        \end{checkboxes}
    }
...
\begin{enumerate}
    ...
    \begin{checkboxes}
        ...
        \CorrectChoice With probability at least $1-\delta$, every hypothesis with training error 0 has true error at most $\epsilon$.
    ...
    \begin{tcolorbox}[fit,height=1cm, width=15cm, blank, borderline={1pt}{-2pt},nobeforeafter]
	    ...
	    $\hat R + O(sqrt(\frac{1}{N} ( 16 + ln(\frac{1}{\delta}))))$
    %solution
    \end{tcolorbox} 
...
\begin{enumerate}
    ...
    \textbf{Select one:}
    \begin{checkboxes}
        \CorrectChoice Yes, because the derivative of the negative conditional log-likelihood has the same form as the derivative of the MSE loss. 
    ...
    \begin{checkboxes}
        ...
        \choice $\log p(D|\textbf{w}) + \sum_{m=1}^M -\log(\sqrt{2\pi\lambda}) - \lambda (w_m)^2$
        \CorrectChoice $\log p(D|\textbf{w}) -  \sum_{m=1}^M \log(\sqrt{\frac{2\pi}{\lambda}}) - \frac{\lambda}{2}(w_m)^2$
    ...
    \textbf{Select one:}
    \begin{checkboxes}
        \CorrectChoice Increase variance $\sigma^2$
    ...
    \begin{checkboxes}
        ...
        \CorrectChoice $- \ell(\mathbf{w}) + \frac{\lambda}{2}\|\mathbf{w}\|_2^2$
    ...
    \textbf{Select one:}
    \begin{checkboxes}
        \CorrectChoice Uniform distribution over $[- \mathbf{w}^T\mathbf{x}^{(i)}, \mathbf{w}^T\mathbf{x}^{(i)} ]$
...
\begin{enumerate}
    ...
    {\checkboxchar{$\Box$} \checkedchar{$\blacksquare$}
        ...
        \CorrectChoice This will not raise a problem in the output  $P(Y|X_1,X_2,X_3,X_4)$ because the conditional independence assumption will correctly treat this situation.
        \choice None of the above
    \end{checkboxes}
    }
...
\textbf{Collaboration Questions} Please answer the following:
    ...
    \begin{enumerate}
        \item Did you receive any help whatsoever from anyone in solving this assignment? Is so, include full details. NO
